// TEST
/* Follow the guidelines in the comments */
/* Some code is incorrect and needs fixing */

/* 1. Define a string called name and make it equal to your own name  */
const name = 'Martin';
/* 2. Define a number called age and make it equal to your own age */
const age = '34';
/* 3. Define a string called hobby and make it equal to your favorite hobby */
const hobby = 'baseball';
/* 4. Fix this console log so that it correctly concatenates your string */
console.log(
  'My name is ' +
    name +
    ' I am ' +
    age +
    ' years old and love ' +
    hobby
);
/* 5. This console.log should be printing out 25, fix it */
let num = 25;

if (num === 25) {
  console.log(num);
}

/* 6. Something is wrong with this shopping list if you want 4 items in the list */
const shoppingList = ['apples', 'bananas', 'pears', 'lettuce'];

/* 7. Now that you have fixed the shopping list, lets print out each individual item to the console*/

console.log(
  shoppingList[0],
  shoppingList[1],
  shoppingList[2],
  shoppingList[3]
);

console.log(shoppingList.indexOf('lettuce')); //3
